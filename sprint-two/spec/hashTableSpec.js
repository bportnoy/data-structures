describe('hashTable', function() {
  ...
  it('should double in size when needed', function() {
    hashTable.insert('Ben','Creasy');
    hashTable.insert('Bradley','Portnoy');
    _.each(people, function(person) {
      var firstName = person[0], lastName = person[1];
      hashTable.insert(firstName,lastName);
    });
    expect(hashTable._limit).to.equal(16);
    hashTable.remove('Bradley');
    expect(hashTable.retrieve('Ben')).to.equal('Creasy');
    expect(hashTable.retrieve('Bradley')).to.equal(null);
  });
